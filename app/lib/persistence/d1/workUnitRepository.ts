...
export class D1WorkUnitRepository implements WorkUnitRepository {
  ...
  async create(_ctx: TenantDbContext, row: InboxWorkUnitRow): Promise<InboxWorkUnitRow> {
    ...
    ).bind(row.id, row.tenantId, row.sourceSignalId ?? null, row.title, row.kind, row.priority, row.sourceProvider, row.reason, row.evidence, row.nextAction, row.sourceUrl ?? null, row.actor ?? null, row.assignee ?? null, row.repository ?? null, row.dueAt ?? null, row.status, row.createdAt, row.updatedAt).run()
    return row
  }
  ...
  async findById(_ctx: TenantDbContext, id: string): Promise<InboxWorkUnitRow | null> {
    const row = await this.db.prepare("SELECT * FROM work_units WHERE id = ?").bind(id).first<Record<string, unknown>>()
    if (!row) return null
    if (String(row.tenant_id) !== String(_ctx.tenantId)) return null
  ...
  async listRecent(_ctx: TenantDbContext, limit = 50): Promise<InboxWorkUnitRow[]> {
    const rows = await this.db.prepare("SELECT * FROM work_units ORDER BY created_at DESC LIMIT ?").bind(limit).all<Record<string, unknown>>()
    return (rows.results ?? []).map(mapWorkUnit).filter((row) => String(row.tenantId) === String(_ctx.tenantId))
  }
}
