...
- 候補情報を低摩擦で取り込む
- AI が少数の候補だけを前面に出す
- ユーザーが `keep / discard / open` を高速に判断する
- その判断結果を学習データとして保存する
...
- AI による要約とノイズ圧縮
- デスクトップ上での少数カードの高速トリアージ
- `keep / discard / open` の明示的な判断操作
- 判断ログの構造化保存
...
長期的な WorkUnit OS の UI は、3カラム構造を基本とする。

- `Inbox` : 情報ソースと要約
- `Tasks / WorkUnit` : 優先度付きの判断対象と作業対象
- `Studio` : メモ、編集、統合、思考整理

### 8A. Hopper の操作モデル

短期の Hopper UI は、広いワークスペースよりも高速判断を優先する。

想定動作:

- アプリを開いた時点で、AI が選んだ少数カードが読み込まれている
- 各カードには、ソース、タイトル、3行要約、タグ、表示理由を出す
- `keep` : 保持して後続処理に送る
- `discard` : 破棄し、今後の学習に負例として使う
- `open` : 元ソースまたは詳細を開く
...
`Share Sheet / Browser Extension / X / GitHub / RSS -> n8n -> LLM -> D1 -> Hopper / Inbox / WorkUnit / Studio -> Issue / Commit / PR`
...
`Share Sheet / Browser Extension -> n8n -> LLM -> D1 -> Hopper -> keep / discard / open`

### X

`X Bookmark -> n8n -> LLM -> D1 -> Hopper -> keep / discard / open`

### GitHub

`GitHub Star -> n8n -> Repo Fetch / README Extract / LLM -> D1 -> Hopper -> WorkUnit -> Issue -> Commit -> PR`

### RSS / Newsletter

`RSS / Newsletter -> n8n -> Content Fetch -> LLM -> D1 -> Hopper -> keep / discard / open`

### Screenshot

`Screenshot -> OCR / Source Resolution -> n8n -> LLM -> D1 -> Hopper`
...
`Hopper の判断データ -> 文脈ランキング -> WorkUnit 生成 -> 実行自動化`
...
`Inbox -> WorkUnit -> Issue -> Commit -> PR`
